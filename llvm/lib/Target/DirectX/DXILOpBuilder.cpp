...
CallInst *DXILOpBuilder::createCreateHandle(Type *OverloadTy, int8_t ResClass,
                                            int RangeID, Value *Index,
                                            bool NonUniformIndex) {
  auto Fn = createDXILOpFunction(DXIL::OpCode::CreateHandle, OverloadTy, M);
  return B.CreateCall(Fn, {B.getInt32((int32_t)DXIL::OpCode::CreateHandle),
                           B.getInt8(ResClass), B.getInt32(RangeID), Index,
                           B.getInt1(NonUniformIndex)});
}

CallInst *DXILOpBuilder::createDXILOpCall(DXIL::OpCode OpCode, Type *OverloadTy,
                                          llvm::iterator_range<Use *> Args) {
  auto Fn = createDXILOpFunction(OpCode, OverloadTy, M);
...
Type *DXILOpBuilder::getOverloadTy(DXIL::OpCode OpCode, FunctionType *FT,
  ...
  if (Prop->OverloadParamIndex != 0) {
    // Skip Return Type and Type for DXIL opcode.
    const unsigned SkipedParam = NoOpCodeParam ? 1 : 2;
  ...
  if (Kind == ParameterKind::CBUFFER_RET || Kind == ParameterKind::RESOURCE_RET) {
